...
\Shorttitle{\fme: inverse modelling, sensitivity,
  Monte Carlo with dynamic models}
...
%\VignetteIndexEntry{Sensitivity, calibration, identifiability, Monte Carlo analysis of a dynamic simulation model}
...
plot(out$time,out$Bact,ylim=range(c(out$Bact,out$Sub)),
     xlab="time, hour",ylab="mol C/m3",type="l",lwd=2)
...
The best-fit model is run, the model cost function estimated and the
model outcome compared to data.
