...
\author{Karline Soetaert\\Netherlands Institute of Ecology
...
%% MCMC is rather time consuming, so we don't run this now
<<>>=
...
%%but instead load the pre-calculated data
